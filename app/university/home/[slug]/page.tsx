...
export default async function UniversityHomePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      <UniversityNavbar name={university.name} logo={university.logo} slug={slug} />
          ...
          <div className="flex items-start gap-5">
            {university.logo ? (
              ...
              <img
                src={university.logo}
                alt={university.name}
                className="w-20 h-20 rounded-xl object-contain border border-gray-100 bg-gray-50 p-1.5 flex-shrink-0"
              />
            ) : (
            ...
            <div className="flex-1">
              <div className="flex items-center gap-3 flex-wrap mb-2">
                ...
                <Badge
                  label={university.verificationStatus === 'verified' ? 'Verified' : 'Pending'}
                  variant={university.verificationStatus === 'verified' ? 'success' : 'warning'}
                />
              </div>
              {university.description && (
                <p className="text-sm text-gray-500 mb-3 leading-relaxed">{university.description}</p>
              )}
              <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                ...
                <span className="flex items-center gap-1.5"><Users className="w-3.5 h-3.5" /> {students.length} verified students</span>
      ...
      <UniversityFooter name={university.name} logo={university.logo} website={university.website} />
    </div>
  );
}
